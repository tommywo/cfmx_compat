...
public final class CFMXCompat {
  ...
  public String encrypt(String plain, String key, String encoding) {
    Tranfromer tranfromer = new Tranfromer();
    switch (encoding.toLowerCase()){
      case UU:
        return UUCoder.encode(tranfromer.transformString(key, plain.getBytes())).trim();
      case HEX:
        return HexCoder.encode(tranfromer.transformString(key, plain.getBytes())).trim();
      case BASE64:
        return Base64Coder.encode(tranfromer.transformString(key, plain.getBytes())).trim();
      default:
        return null;
    }
  }
  ...
  public String decrypt(String encrypted, String key, String encoding) throws CoderException {
      ...
      switch (encoding.toLowerCase()) {
        case UU:
          return new String(tranfromer.transformString(key, UUCoder.decode(encrypted)));
        case HEX:
          return new String(tranfromer.transformString(key, HexCoder.decode(encrypted)));
        case BASE64:
          return new String(tranfromer.transformString(key, Base64Coder.decode(encrypted)));
        default:
          return null;
      }
  }
